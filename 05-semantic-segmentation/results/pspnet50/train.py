...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # print("In train function, input shape: ", input.shape)
        # print("In train function, ouput shape: ", output.shape)
        # print("In train function, target shape: ", target.shape)
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # print("In validate function, before ouput shape: ", output.shape)

        output = output.max(1)[1]
        # print("In validate function, input shape: ", input.shape)
        # print("In validate function, ouput shape: ", output.shape)
        # print("In validate function, target shape: ", target.shape)
        intersection, union, target = intersectionAndUnionGPU(output, target, args.classes, args.ignore_label)
        if args.multiprocessing_distributed:
            dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(target)
        intersection, union, target = intersection.cpu().numpy(), union.cpu().numpy(), target.cpu().numpy()
