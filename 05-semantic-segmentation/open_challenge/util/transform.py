...
class AdaptImage(object):
    ...
    def __call__(self, image, label):
        h, w, c = image.shape[:]
        assert h == 512 and w == 1024, 'invalide h: {}, w: {}'.format(h, w)
        image_mask = np.zeros((513, 1025, c)).astype(image.dtype)
        label_mask = np.ones((513, 1025)).astype(label.dtype) * self.ignore_label
        image_mask[:h, :w, :c] = image
        label_mask[:h, :w] = label
