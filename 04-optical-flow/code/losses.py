...
class EPELoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        square_pixel = torch.square(output-target)
        sqrt_sum = torch.sqrt(torch.sum(square_pixel,dim=1))
        epevalue = sqrt_sum.mean()       
...
class MultiscaleLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        for i, output_ in enumerate(output):
            ...
            ''' Implement the MultiScale loss here'''
            square_pixel = torch.square(output_-target_)
            sqrt_sum = torch.sqrt(torch.sum(square_pixel,dim=1))
            lossvalue = sqrt_sum.mean()
